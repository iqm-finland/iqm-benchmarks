# QED-C Application-Oriented Benchmarks hosted in QCVV suite

This is a reproduction of the Application-Oriented benchmark repository:<br>https://github.com/SRI-International/QC-App-Oriented-Benchmarks

The package is provisionally reproduced here for convenience. Additionally, any changes that have been made are listed below. The techniques can be seamlessly executed via the `benchmarks_qiskit_IQM.ipynb` notebook.

<ins>Changes</ins>:
* Git config files and Jupyter notebooks other than qiskit's have been removed.
* Qiskit's jupyter notebook was modified with relevant example parameters.
* IQM `backend` objects can be fetched from qcvv_suite `utils` file simply by specifying a name (e.g. "garnet", "fakeapollo", etc).
* The `optimize_single_qubit_gates` function can be used by setting `"optimize_sqg": True` in the `exec_options` dictionary.
* `basis_gates` was reduced to `['r', 'cz']` - support for Deneb (include `move` gate) to be tested separately.
* Reformatting of the code with black to improve readability

Other than the changes listed above, the source has not been modified.<br>
* In particular, routing and layout choice has been left as originally coded (e.g., there is no enforcement of a "fixed physical layout") and uses qiskit's `layout_method`.
* Future improvements could include choosing specific layouts and enforcing a fixed physical layout routing.
