...
class HamiltonianKernel(object):
    ...
    def __init__(
        self, n_spins, K, t, hamiltonian, w, hx, hz, use_XX_YY_ZZ_gates, method, random_pauli_flag, init_state
    ):
    ...
    def kernel_draw(self):
        ...
        HamiltonianKernel.QC_ = HamiltonianKernel.QCI_ = HamiltonianKernel.QCH_ = HamiltonianKernel.QC2D_ = (
            HamiltonianKernel.QCRS_
        ) = None
...
class HeisenbergHamiltonianKernel(HamiltonianKernel):
        ...
        for k in range(self.K):
            if self.use_XX_YY_ZZ_gates:
                ...
                for j in reversed(range(2)):
                    for i in reversed(range(j % 2, self.n_spins - 1, 2)):
                        qc.append(
                            xxyyzz_opt_gate_mirror(self.tau).to_instruction(), [qr[i], qr[(i + 1) % self.n_spins]]
                        )
            qc.barrier()
            [qc.rz(-2 * self.tau * self.w * self.h_z[i], qr[i]) for i in range(self.n_spins)]
            [qc.rx(-2 * self.tau * self.w * self.h_x[i], qr[i]) for i in range(self.n_spins)]
    ...
    def create_quasi_inverse_hamiltonian(self) -> QuantumCircuit:
        ...
        self.QCRS_ = res_pauli = (
            self.ResultantPauli()
        )  # create a resultant pauli that we want to apply to initial state.
                ...
                for j in reversed(range(2)):
                    ...
                    for i in reversed(range(j % 2, self.n_spins - 1, 2)):
                        if k == 0 and j == 1:
                            ...
                            qc.append(
                                xxyyzz_opt_gate_quasi_mirror(self.tau, gate_i, gate_next).to_instruction(),
                                [qr[i], qr[(i + 1) % self.n_spins]],
                            )
