...
@pytest.mark.parametrize(
    "n_spins,hamiltonian",
    [(spin, hamiltonian) for spin in range(2, 13) for hamiltonian in ["Heisenberg", "TFIM"]],
)
...
def test_qiskit_matches_manual(n_spins, hamiltonian):
    ...
    if n_spins == 2 and hamiltonian == "Heisenberg":
        ...
        transpile(qiskit_circuit, ex.backend, basis_gates=["rx", "ry", "rz", "cx"]).draw(
            filename="qiskitcirq", output="mpl"
        )
