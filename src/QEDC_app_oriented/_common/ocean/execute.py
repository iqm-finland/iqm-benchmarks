    ...
    try:
        ...
        if device_name == "pegasus":
            ...
            sampleset = sampler.sample_ising(
                qc.h, qc.J, num_reads=shots, num_sweeps=num_sweeps, annealing_time=annealing_time
            )
            sampleset.resolve()
        ...
        # execute on D-Wave hardware
            ...
            if verbose_time:
                print(json.dumps(sampleset.info["timing"], indent=2))
        ...
        logger.info(
            f'Finished Running ocean.execute() - elapsed, exec, opt time = {elapsed_time} {exec_time} {opt_exec_time} (sec)'
        )
        if verbose_time:
            print(f"  ... ocean.execute() elapsed, exec, opt time = {elapsed_time}, {exec_time}, {opt_exec_time} (sec)")
