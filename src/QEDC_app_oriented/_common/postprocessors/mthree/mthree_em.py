...
def mthree_handler(result):
    ...
    qrange = qmax - qmin
    if verbose:
        print(f"{qmin = } {qmax = } {qrange = } {qshots = } {shots = }")
...
def mthree_width_handler(circuit):
    ...
    if num_qubits != mit_num_qubits:
        if verbose:
            print(f'... updating mthree width to {num_qubits = }')
...
def get_mthree_handlers(backend_id, provider_backend):
    ...
    if verbose:
        print(f"... initializing mthree for backend_id = {backend_id}")
