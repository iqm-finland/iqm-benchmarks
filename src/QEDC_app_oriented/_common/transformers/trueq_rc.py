...
class Native1QSparse(tqc.Parallel):
    def __init__(self, factories, mode="ZXZXZ", angles=None, **_):
        ...
        replacements = [tqc.NativeExact(factories), tqc.Native1QMode(factories)]
...
compiler = tq.Compiler(
    [tqc.Native2Q(factories), tqc.UnmarkCycles(), tqc.Merge(), tqc.RemoveId(), tqc.Justify(), tqc.RemoveEmptyCycle()]
)

compiler_local_RC = tq.Compiler([tqc.MarkBlocks(), tqc.RCLocal(), Native1QSparse(factories)])

compiler_full_RC = tq.Compiler([tqc.MarkBlocks(), tqc.RCCycle(), Native1QSparse(factories)])
