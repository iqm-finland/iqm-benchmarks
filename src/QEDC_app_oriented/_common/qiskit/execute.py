...
def set_execution_target(
    ...
    if provider_backend != None:
        ...
        if hub == "azure-quantum":
            ...
            session = backend.open_session(name=session_name, job_failure_policy=SessionJobFailurePolicy.CONTINUE)
        ...
        # If hub variable indicates an Azure Quantum backend
            ...
            if azure_provider is None:
                ...
                azure_provider = AzureQuantumProvider(
                    resource_id=os.getenv("AZURE_QUANTUM_RESOURCE_ID"), location=os.getenv("AZURE_QUANTUM_LOCATION")
                )
        ...
        # normal execution processing is performed here
            ...
            if this_noise is not None and not use_sessions and backend_name.endswith("qasm_simulator"):
                ...
                trans_qc = transpile_and_bind_circuit(
                    circuit["qc"], circuit["params"], backend, basis_gates=this_noise.basis_gates
                )
                ...
                if transformer:
                    ...
                    simulation_circuits, shots = invoke_transformer(transformer, trans_qc, backend=backend, shots=shots)
            ...
            # Initiate execution for all other backends and noiseless simulator
                ...
                # if set, transpile many times and pick the shortest circuit
                ...
                # apply transformer pass if provided
                if transformer:
                    trans_qc, shots = invoke_transformer(transformer, trans_qc, backend=backend, shots=shots)
                ...
                ##################################################
                ##################################################
                # OPTIMIZE SQG FOR IQM BACKENDS HERE!
                if "optimize_sqg" in backend_exec_options_copy:
                    # might need a conditional asking that backend is IQM's
                    # can't think of something other than querying backend.name
                    if backend_exec_options_copy["optimize_sqg"]:
                        trans_qc = optimize_single_qubit_gates(trans_qc)
                ##################################################
                ##################################################
...
def transpile_and_bind_circuit(
    circuit, params, backend, basis_gates=None, optimization_level=None, layout_method=None, routing_method=None
):
...
def transpile_multiple_times(
    circuit, params, backend, transpile_attempt_count, optimization_level=None, layout_method=None, routing_method=None
):
