...
group_metrics = {
    ...
    "avg_exec_validating_times": [],
    "avg_exec_running_times": [],
    "job_ids": [],
}
    ...
    if group in group_metrics["groups"]:
        ...
        if group_index >= 0:
            ...
            if len(group_metrics["avg_depths"]) > 0:
                ...
                if avg_depth > 0:
                    print(
                        f"Average Circuit Algorithmic Depth, \u03BE (xi) for the {group} qubit group = {int(avg_depth)}, {avg_xi}"
                    )
            ...
            if len(group_metrics["avg_tr_depths"]) > 0:
                ...
                if avg_tr_depth > 0:
                    print(
                        f"Average Normalized Transpiled Depth, \u03BE (xi), 2q gates for the {group} qubit group = {int(avg_tr_depth)}, {avg_tr_xi}, {avg_tr_n2q}"
                    )
            ...
            print(
                f"Average Creation, Elapsed, Execution Time for the {group} qubit group = {avg_create_time}, {avg_elapsed_time}, {avg_exec_time} secs"
            )
            ...
            print(
                f"Average Hellinger, Normalized Fidelity for the {group} qubit group = {avg_hf_fidelity}, {avg_fidelity}"
            )
...
def get_aq_width(shared_data, w_min, w_max, fidelity_metric):
    ...
    for app in shared_data:
        ...
        while True:
            ...
            fail_w = [
                i
                for i in range(len(n2q_data))
                if (float(n2q_data[i]) <= n2q_cutoff and float(w_data[i]) <= AQ and float(fidelity_data[i]) < aq_cutoff)
            ]
            if len(fail_w) == 0:
                break
            AQ -= 1
...
def plot_metrics(
    suptitle="Circuit Width (Number of Qubits)",
    transform_qubit_group=False,
    new_qubit_group=None,
    filters=None,
    suffix="",
    options=None,
):
    ...
    if save_metrics:
        ...
        if transform_qubit_group:
            ...
            store_app_metrics(
                backend_id, circuit_metrics, group_metrics, suptitle, start_time=start_time, end_time=end_time
            )
    ...
    if filters != None:
        if "create" not in filters:
            do_creates = False
        if "execute" not in filters:
            do_executes = False
        if "fidelity" not in filters:
            do_fidelities = False
        if "hf_fidelity" not in filters:
            do_hf_fidelities = False
        if "depth" not in filters:
            do_depths = False
        if "2q" not in filters:
            do_2qs = False
        if "vbplot" not in filters:
            do_vbplot = False

        # this is a way to turn these on, if aq_mode not used
        if "hf_fidelity" in filters:
            do_hf_fidelities = True
        if "2q" in filters:
            do_2qs = True
    ...
    if do_executes:
        ...
        avg_elapsed_times = modify_elapsed_times(avg_elapsed_times, avg_exec_creating_times, avg_exec_times)
    ...
    if {do_depths or do_2qs} and do_volumetric_plots and do_vbplot:
        ...
        try:
            ...
            if aq_mode > 0:
                ax = plot_volumetric_background_aq(
...
def plot_metrics_all_merged(
    shared_data,
    backend_id,
    suptitle=None,
    imagename="_ALL-vplot-2",
    avail_qubits=0,
    is_individual=False,
    score_metric=None,
    max_depth=0,
    suppress_low_fidelity=False,
):
    ...
    try:
        ...
        if not is_individual:
            plot_merged_result_rectangles(
                ...
                max_depth=max_depth,
                suppress_low_fidelity=suppress_low_fidelity,
            )
        ...
        for app in shared_data:
            ...
            if aq_mode > 0:
                if score_metric not in group_metrics:
                    continue
...
def plot_merged_result_rectangles(
    shared_data, ax, max_qubits, w_max, num_grads=4, score_metric=None, max_depth=0, suppress_low_fidelity=False
):
...
def plot_all_app_metrics(
    ...
    if do_volumetric_plots:
        ...
        plot_metrics_all_merged(
            shared_data,
            backend_id,
            suptitle=fulltitle,
            imagename="_ALL-vplot-2" + suffix,
            avail_qubits=avail_qubits,
...
def save_plot_image(plt, imagename, backend_id):
    ...
    if not os.path.exists('__images'):
        os.makedirs('__images')
    if not os.path.exists(f'__images/{backend_id}{data_suffix}'):
        os.makedirs(f'__images/{backend_id}{data_suffix}')
    ...
    for group in circuit_metrics_detail_2:
        ...
        for circuit_id in [restart_index]:  # circuit_metrics_detail_2[group]:
            ...
            for it in metrics_array:
                ...
                if x_metric not in mets:
                    break
                if score_metric not in mets:
                    break
    ...
    for i in range(len(values)):
        ...
        if i > 0:
            ...
            if delta == 0:
                ...
                if fill and gap > 1:
                    basis.append(None)  # put space between items if gap > 1
                basis.append(values[i])
...
def plot_ECDF(suptitle="", options=None, suffix=None):
    """
    ...
    with plt.style.context(maxcut_style):
        ...
        for group in circuit_metrics_final_iter:
            ...
            for restart_ind in [best_restart_ind]:  # circuit_metrics_final_iter[group]:
                ...
                axs.plot(
                    np.array(unique_sizes) / optimal_value,
                    np.array(cumul_counts) / cumul_counts[-1],
                    marker='o',
                    # ls = '-', label = f"Width={group}")#" degree={deg}") # lw=1,
                    ls='-',
                    label=f"Problem Size={group}",
                )  # " degree={deg}") # lw=1,
...
def plot_cutsize_distribution(
    suptitle="Circuit Width (Number of Qubits)", list_of_widths=[], options=None, suffix=None
):
    """
...
def get_distribution_and_stats():
    ...
    Returns:
        dictionary
    """
    group_metrics_optgaps = {
        'approx_ratio': {'color': 'r', 'label': 'Approx. Ratio', 'gapvals': [], 'ratiovals': []},
    ...
    for group in groups:
        ...
        for circuit_id in [best_restart_ind]:  # circuit_metrics_detail_2[group]:
            ...
            full_counts_list = [
                unique_counts[unique_sizes.index(s)] if s in unique_sizes else 0 for s in full_size_list
            ]
            group_metrics_optgaps['cutsize_ratio_dist']['ratios'].append(np.array(full_size_list) / optimal_value)
            group_metrics_optgaps['cutsize_ratio_dist']['frequencies'].append(
                np.array(full_counts_list) / sum(full_counts_list)
            )
            ...
            full_size_list = list(range(optimal_value + 1))
            full_counts_list_unif = [
                unique_counts_unif[unique_sizes_unif.index(s)] if s in unique_sizes_unif else 0 for s in full_size_list
            ]
            group_metrics_optgaps['random_cutsize_ratio_dist']['ratios'].append(
                np.array(full_size_list) / optimal_value
            )

            # hack to avoid crash if all zeros
            sum_full_counts_list_unif = sum(full_counts_list_unif)
            if sum_full_counts_list_unif <= 0:
                sum_full_counts_list_unif = 1

            group_metrics_optgaps['random_cutsize_ratio_dist']['frequencies'].append(
                np.array(full_counts_list_unif) / sum_full_counts_list_unif
            )
...
def plot_metrics_optgaps(
    ...
    with plt.style.context(maxcut_style):
        ...
        # Plot quartiles
        q_vals = group_metrics_optgaps['quantile_optgaps'][
            'gapvals'
        ]  # list of lists; shape (number of circuit widths, 3)
    ...
    with plt.style.context(maxcut_style):
        ...
        if 'quantile_optgaps' in which_metrics_to_plot:
            ...
            plt_handles['quantile_optgaps'] = axs.errorbar(
                xvalues,
                center_optgaps,
                yerr=errors,
                ecolor='k',
        ...
        for metric_str in set(which_metrics_to_plot) - set(["quantile_optgaps", "violin"]):
            ...
            (plt_handles[metric_str],) = axs.plot(
                xvalues,
                group_metrics_optgaps[metric_str]['gapvals'],
                marker='o',
                lw=1,
                ls=ls,
        ...
        axs.legend(
            handles=handles_list, ncol=2, loc='upper right'
        )  # loc='center left', bbox_to_anchor=(1, 0.5)) # For now, we are only plotting for degree 3, and not -3
...
def set_custom_cmap_style(fade_low_fidelity_level=default_fade_low_fidelity_level, fade_rate=default_fade_rate):
...
def create_custom_spectral_cmap(fade_low_fidelity_level=default_fade_low_fidelity_level, fade_rate=default_fade_rate):
...
def box4_at(x, y, value, type=1, fill=True, alpha=1.0):
    ...
    return Rectangle(
        (x - size / 8, y - size / 2), size / 4, size, alpha=alpha, edgecolor=ec, facecolor=fc, fill=fill, lw=0.1
    )
...
def qv_box_at(x, y, qv_width, qv_depth, value, depth_base):
    ...
    return Rectangle(
        (x - 0.5, y - 0.5),
        depth_index(qv_depth, depth_base),
        qv_width,
...
def plot_volumetric_background(
    max_qubits=11, QV=32, depth_base=2, suptitle=None, avail_qubits=0, colorbar_label="Avg Result Fidelity"
):
    ...
    if QV0 != 0:
        t = ax.text(
            ...
            color=(0.2, 0.2, 0.2),
            bbox=dict(boxstyle="square,pad=0.3", fc=(0.9, 0.9, 0.9), ec="grey", lw=1),
        )
...
def plot_volumetric_background_aq(
    max_qubits=11, AQ=12, depth_base=2, suptitle=None, avail_qubits=0, colorbar_label="Avg Result Fidelity"
):
...
def plot_metrics_background(suptitle, ylabel, x_label, score_label, y_max, x_max, y_min=0, x_min=0, ylabels=None):
    ...
    cbar = plt.colorbar(
        cm.ScalarMappable(cmap=cmap, norm=cmap_norm), cax=None, ax=ax, shrink=0.6, label=score_label, panchor=(0.0, 0.7)
    )
    if score_label == 'Accuracy Volume':
...
def plot_volumetric_data(
    ...
    if do_label:
        ax.annotate(
            label,
            xy=(x_anno + labelpos[0], y_anno + labelpos[1]),
            rotation=labelrot,
...
def anno_volumetric_data(ax, depth_base=2, label='Depth', labelpos=(0.2, 0.7), labelrot=0, type=1, fill=True):
    ...
    for i in range(len(anno_labels)):
        ...
        ax.annotate(
            ...
            arrowprops=dict(
                facecolor='black', shrink=0.0, width=0.5, headwidth=4, headlength=5, edgecolor=(0.8, 0.8, 0.8)
            ),
