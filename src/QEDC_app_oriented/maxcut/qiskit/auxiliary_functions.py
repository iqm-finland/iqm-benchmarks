...
def plot_worst_best_init_conditions(worst_dict, best_dict):
    ...
    with plt.style.context(maxcut_style):
        ...
        color = clr_random
        uniform_approx_ratio = (
            -maxcut_benchmark.compute_sample_mean(worst_dict['unique_counts_unif'], worst_dict['unique_sizes_unif'])
            / optimal_value
        )
        axs.plot(
            np.array(unique_sizes_unif) / optimal_value,
            np.array(unique_counts_unif) / sum(unique_counts_unif),
            marker='o',
            ms=1,
            mec='k',
            mew=0.2,
        ...
        fin_appr_ratio = (
            -maxcut_benchmark.compute_sample_mean(best_dict['unique_counts'], best_dict['unique_sizes']) / optimal_value
        )
        color = colors[1]
        axs.plot(
            np.array(unique_sizes) / optimal_value,
            np.array(unique_counts) / sum(unique_counts),
            marker='o',
            ls='-',
            ms=4,
            mec='k',
        ...
        optimal_value = worst_dict['optimal_value']
        fin_appr_ratio = (
            -maxcut_benchmark.compute_sample_mean(worst_dict['unique_counts'], worst_dict['unique_sizes'])
            / optimal_value
        )
        color = colors[0]
        axs.plot(
    ...
    for ind in range(2 * rounds):
        ...
        with plt.style.context(maxcut_style):
            ...
            restart_dots = ax.scatter(
                ...
                marker='o',
                alpha=0.8,
                label=r'Converged Angles'.format(),
                linewidths=0,
            )
            ...
            cbar = plt.colorbar(restart_dots, location='left', shrink=0.8, orientation='vertical', aspect=15)
...
def rearrange_radar_data_for_plotting(gen_prop):
    ...
    for width_ind, width in enumerate(widths):
        ...
        for iter_ind in range(niter):
            ...
            arranged_init_AR_arr[width_ind, iter_ind] = radar_data[width]['init_AR'][iter_ind]
            arranged_initthetas_arr[width_ind, iter_ind, :] = radar_data[width]['init_angles'][iter_ind]

    return dict(
        arranged_AR_arr=arranged_AR_arr,
...
def get_radar_data_from_metrics():
    ...
    radar_data = {width: dict(ARs=[], init_angles=[], final_angles=[], init_AR=[]) for width in widths}
