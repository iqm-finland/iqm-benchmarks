...
def analyze_and_print_result(qc, result, num_qubits, secret_int, num_shots):
    ...
    if verbose:
        print(f"For secret int {secret_int} fidelity: {fidelity}")
...
def uniform_cut_sampling(num_qubits, degree, num_shots, _instances=None):
    ...
    instance_filename = os.path.join(
        os.path.dirname(__file__), "..", "_common", common.INSTANCE_DIR, f"mc_{num_qubits:03d}_{degree:03d}_000.txt"
    )
...
def save_runtime_data(
    result_dict,
):  # This function will need changes, since circuit metrics dictionaries are now different
    ...
    for width in detail_2:
        ...
        for restart_ind in restart_ind_list:
            ...
            instance_filename = os.path.join(
                ...
                "_common",
                common.INSTANCE_DIR,
                f"mc_{int(width):03d}_{int(degree):03d}_000.txt",
            )
            ...
            if benchmark_inputs['save_final_counts']:
                ...
                iter_dist = {
                    'cuts': finIterDict['cuts'],
                    'counts': finIterDict['counts'],
                    'sizes': finIterDict['sizes'],
                }
            else:
            ...
            iter_size_dist = {
                'unique_sizes': finIterDict['unique_sizes'],
                'unique_counts': finIterDict['unique_counts'],
                'cumul_counts': finIterDict['cumul_counts'],
            }
...
def store_final_iter_to_metrics_json(
    ...
    }  # store only the distribution of cut sizes, and not the cuts themselves
...
iter_dist = {
    ...
    'counts': [],
    'sizes': [],
}  # (list of measured bitstrings, list of corresponding counts, list of corresponding cut sizes)
iter_size_dist = {
    'unique_sizes': [],
    'unique_counts': [],
    'cumul_counts': [],
}  # for the iteration being executed, stores the distribution for cut sizes
saved_result = {}
...
def run(
    ...
    if detailed_save_names:
        ...
        parent_folder_save = os.path.join(
            '__results', f'{backend_id}', objective_func_type, f'run_start_{start_time_str}'
        )
    else:
    ...
    if use_fixed_angles:
        ...
        fixed_angles = common.read_fixed_angles(
            os.path.join(os.path.dirname(__file__), '..', '_common', 'angles_regular_graphs.json'), _instances
        )
    ...
    if method == 1:
        metrics.plot_metrics(
            f"Benchmark Results - {benchmark_name} ({method}) - Qiskit", options=dict(shots=num_shots, rounds=rounds)
        )
    elif method == 2:
