...
def sample_mirror_circuits(c, num_mcs=100, randomized_state_preparation=True, rand_state=None, random_pauli=True):
    ...
    for j in range(num_mcs):
        mc, bs = central_pauli_mirror_circuit(
            c, randomized_state_preparation=randomized_state_preparation, random_pauli=True, rand_state=rand_state
        )
...
def sample_reference_circuits(qubits, num_ref=100, randomized_state_preparation=True, rand_state=None):
    ...
    for j in range(num_ref):
        ...
        ref_c, bs = central_pauli_mirror_circuit(
            empty_circuit, randomized_state_preparation=randomized_state_preparation, rand_state=rand_state
        )
...
def central_pauli_mirror_circuit(circ, randomized_state_preparation=True, random_pauli=True, rand_state=None):
    ...
    mc = (
        circuit_to_mirror
        + pygsti.circuits.Circuit([central_pauli_layer], line_labels=circ.line_labels)
        + quasi_inverse_circ
    )
    mc.done_editing()
...
def adjusted_success_probability(hamming_distance_counts):
    if np.sum(hamming_distance_counts) == 0.0:
        return 0.0
    else:
...
def predicted_process_fidelity(mirror_circuit_effective_pols, reference_effective_pols, n):
    ...
    if c <= 0.0:
        return (
            np.nan
        )  # raise ValueError("Reference effective polarization zero or smaller! Cannot estimate the process fidelity")
    elif a <= 0:
        return 0.0
    else:
...
def convert_to_mirror_circuit(qc, random_pauli, init_state):
    ...
    mcs, bss = sample_mirror_circuits(
        pygsti_circuit, num_mcs=1, random_pauli=random_pauli, randomized_state_preparation=random_init_flag
    )
