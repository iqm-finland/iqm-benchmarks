...
def get_valid_qubits(min_qubits, max_qubits, skip_qubits):
    ...
    for qubits in qubit_candidates:
        ...
        while n_spins <= max_qubits:
            dataset_name_template = dataset_name_template.replace("{ratio}", str(global_ratio)).replace(
                "{rinst}", str(global_rinst)
            )
            dataset_name_template = dataset_name_template.replace("{h}", str(global_h)).replace(
                "{pbc_val}", str(global_pbc_val)
            )
            dataset_name_template = dataset_name_template.replace("{U}", str(global_U)).replace(
                "{enc}", str(global_enc)
            )
            dataset_name = dataset_name_template.replace("{n_qubits}", str(n_spins)).replace(
                "{n_qubits/2}", str(n_spins)
            )
            ...
            if data is not None:
                ...
                n_spins += 1
                if (
                    n_spins >= (qubits + skip_qubits) // 2
                    if "{n_qubits/2}" in dataset_name_template
                    else (qubits + skip_qubits)
                ):
...
def create_circuit(
    ...
    if data is not None:
        ...
        circuit = create_trotter_steps(
            num_trotter_steps, evo if not use_inverse_flag else evo.inverse(), ham_op, circuit
        )

        circuit_without_initial_state = create_trotter_steps(
            num_trotter_steps, evo if not use_inverse_flag else evo.inverse(), ham_op, circuit_without_initial_state
        )
            ...
            # if adding Paulis, do that here, with code from pyGSTi
                ...
                if random_init_flag:
                    circuit, bitstring = convert_to_mirror_circuit(
                        circuit_without_initial_state, random_pauli=True, init_state=None
                    )
                else:
                    init_state = initial_state(n_spins, init_state)
                    circuit, bitstring = convert_to_mirror_circuit(
                        circuit_without_initial_state, random_pauli=True, init_state=init_state
                    )
