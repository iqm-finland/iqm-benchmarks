...
def analyze_and_print_result(qc, result, num_qubits, s_int, num_shots):
    ...
    diag_el = 0.5
    off_diag_el = -0.25
    A = shs.generate_sparse_H(num_input_qubits, off_diag_index, diag_el=diag_el, off_diag_el=off_diag_el)
...
def run(
    ...
    return run2(
        min_input_qubits=min_input_qubits,
        max_input_qubits=max_input_qubits,
        min_clock_qubits=min_clock_qubits,
        max_clock_qubits=max_clock_qubits,
        skip_qubits=skip_qubits,
        max_circuits=max_circuits,
        num_shots=num_shots,
        method=method,
        use_best_widths=use_best_widths,
        min_register_qubits=min_register_qubits,
...
def run2(
    ...
    for num_input_qubits in range(min_input_qubits, max_input_qubits + 1, skip_qubits):
        ...
        for num_clock_qubits in range(min_clock_qubits, max_clock_qubits + 1, skip_qubits):
            ...
            if use_best_widths:
                if num_input_qubits != int((num_qubits - 1) / 3) or num_clock_qubits != (
                    num_qubits - 1 - 2 * num_input_qubits
                ):

                    if verbose:
                        print(
                            f"... SKIPPING {num_circuits} circuits with {num_qubits} qubits, using {num_input_qubits} input qubits and {num_clock_qubits} clock qubits"
                        )
                    continue
            ...
            if (
                min_register_qubits > 1
                and num_input_qubits < min_register_qubits
                or num_clock_qubits < min_register_qubits
            ):
                if verbose:
                    print(
                        f"... SKIPPING {num_circuits} circuits with {num_input_qubits} input qubits and {num_clock_qubits} clock qubits"
                    )
                continue
    ...
    print("\nU^-1 Circuit =")
    print(UI_ if UI_ != None else "  ... too large!")
    print("\nQFT Circuit =")
    print(QFT_ if QFT_ != None else "  ... too large!")
    print("\nInverse QFT Circuit =")
    print(QFTI_ if QFTI_ != None else "  ... too large!")
    print("\nHamiltonian Phase Estimation Circuit =")
    print(HP_ if HP_ != None else "  ... too large!")
    print("\nControlled Rotation Circuit =")
    print(INVROT_ if INVROT_ != None else "  ... too large!")

    # Plot metrics for all circuit sizes
    metrics.plot_metrics(
        f"Benchmark Results - {benchmark_name} - Qiskit",
