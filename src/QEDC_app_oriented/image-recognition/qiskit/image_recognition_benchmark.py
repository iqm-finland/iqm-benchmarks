...
def get_initial_parameters(
    num_qubits: int, ansatz_type: str = 'qcnn uniform', thetas_array=None, reps=1, verbose=False
):
    """
                    ...
                    if i_q2 < num_qubits:
                        qc = qc.compose(
                            parameterized_2q_gate_2(
                                parameter_vector[
                                    num_parameters_per_layer * i_layer : num_parameters_per_layer * (i_layer + 1)
                                ],
                                num_reps=num_reps,
                            ),
                            qubits=(i_q1, i_q2),
                ...
                for i_q1 in range(i_sub_layer, num_qubits, 2 ** (i_layer + 1)):
                    ...
                    if i_q2 < num_qubits:
                        ...
                        qc = qc.compose(
                            parameterized_2q_gate_2(
                                parameter_vector[
                                    num_parameters_per_conv * i_conv : num_parameters_per_conv * (i_conv + 1)
                                ],
                                num_reps=num_reps,
                            ),
                            qubits=(i_q1, i_q2),
...
def calculate_diagonalized_expectation_values(probabilities, observables):
    ...
    for idx, op in enumerate(observables):
        ...
        expectation_value = sampled_expectation_value(probabilities[idx], post_diagonalization_op_converter(op))
...
def run(
    ...
    for num_qubits in range(min_qubits, max_qubits + 1, 2):
        ...
        def objective_function(thetas_array, return_accuracy=False, test_pass=False, train_pass=False):
            ...
            for data_point in x_batch:
                ...
                global quantum_execution_time, quantum_elapsed_time
                if not (train_pass):
                    quantum_execution_time = quantum_execution_time + metrics.get_metric(
                        num_qubits, unique_id, "exec_time"
                    )
            ...
            loss, accuracy = loss_function(
                result=result_array, y_data=y_batch, num_qubits=num_qubits, formatted_observables=frmt_obs
            )
        ...
        if method == 1:
            ...
            thetas_array_0 = get_initial_parameters(
                num_qubits=num_qubits, thetas_array=thetas_array, ansatz_type=ansatz_type, reps=reps
            )
    ...
    if method == 1:
        metrics.plot_metrics(
            f"Benchmark Results - Image Recognition ({method}) - Qiskit", options=dict(shots=num_shots)
        )
