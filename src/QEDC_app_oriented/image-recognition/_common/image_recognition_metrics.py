    ...
    if len(metrics_for_circuit) >= num_average:
        test_accuracy = np.average(
            sorted(
                [
                    (
                        metrics_for_circuit[circuit_id]['test_accuracy']
                        if 'test_accuracy' in metrics_for_circuit[circuit_id]
                        else 0
                    )
                    for circuit_id in metrics_for_circuit
                ],
                reverse=True,
            )[0:num_average]
        )
        train_accuracy = np.average(
            sorted(
                [
                    (
                        metrics_for_circuit[circuit_id]['train_accuracy']
                        if 'train_accuracy' in metrics_for_circuit[circuit_id]
                        else 0
                    )
                    for circuit_id in metrics_for_circuit
                ],
        ...
        train_loss = np.average(
            sorted(
                [
                    (
                        metrics_for_circuit[circuit_id]['train_loss']
                        if 'train_loss' in metrics_for_circuit[circuit_id]
                        else 0
                    )
                    for circuit_id in metrics_for_circuit
                ]
        ...
        train_loss = sorted(
            [
                metrics_for_circuit[circuit_id]['train_loss'] if 'train_loss' in metrics_for_circuit[circuit_id] else 0
                for circuit_id in metrics_for_circuit
            ]
        )[0]
...
def plot_line_metric(
    ...
    x_val: str = 'cumulative_exec_time',
    num_qubits: int = None,
    instance: str = None,
    method=2,
):
    ...
    x_data, x_label, y_data, y_label = find_metrics_array_for_group(
        group, instance, metric_name if not metric_name.endswith("_error") else metric_name[0:-6], x_val, x_metric_name
    )
    ...
    if metric_name.startswith("solution_quality"):
        ...
        x_data_2, x_label_2, y_data_2, y_label_2 = find_metrics_array_for_group(
            group,
            instance,
    ...
    # solution quality error
        ...
        add_two_legend_items(ax, metric_legend_label_2, "darkblue", "-.", metric_legend_label, "darkblue", "-")
    ...
    # accuracy ratio error
        ...
        add_two_legend_items(ax, metric_legend_label, "darkblue", "-", metric_legend_label_2, "darkblue", "-.")
...
def add_two_legend_items(ax, item_label, item_color, item_linestyle, item_label2, item_color2, item_linestyle2):
...
def plot_all_cumulative_metrics(
    suptitle=None,
    ...
    for num_qubits in image_recognition_metrics:
        ...
        for instance in range(1, total_instances + 1):
            ...
            x_data, x_label, y_data, y_label = find_metrics_array_for_group(
                group, instance, "train_accuracy", "cumulative_exec_time", "exec_time"
            )
            ...
            x_data, x_label, y_data, y_label = find_metrics_array_for_group(
                group, instance, "train_accuracy", "cumulative_elapsed_time", "elapsed_time"
            )
            ...
            if (
                len(x_data) > 1
                and omit_initial_elapsed_time_factor > 0
                and (x_data[0] > omit_initial_elapsed_time_factor * x_data[1])
            ):
                x_data[0] = x_data[1]
...
def plot_cumulative_metrics(
    ...
    ax1.plot(x_data, y_data, color='darkblue', linestyle='dotted', linewidth=1, markersize=6, zorder=3)
    ...
    # save the plot image
    if save_plot_images:
        metrics.save_plot_image(plt, os.path.join("Image-Recognition-(2)" + "-" + suffix), backend_id)
    # show the plot(s)
    plt.show(block=True)
...
def plot_exec_time_metrics(
    ...
    ax1.plot(x_data, y_data_2, color='darkblue', linestyle='dotted', linewidth=1, markersize=6, zorder=3)
