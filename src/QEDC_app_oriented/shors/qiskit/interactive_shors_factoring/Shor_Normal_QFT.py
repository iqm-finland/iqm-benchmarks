...
""" Local Imports """
from cfunctions import check_if_power, get_factors, get_value_a
from qfunctions import cMULTmodN, create_inverse_QFT
...
if __name__ == '__main__':
    ...
    while i < len(counts_result):
        print(
            'Result \"{0}\" happened {1} times out of {2}'.format(
                list(sim_result.get_counts().keys())[i], list(sim_result.get_counts().values())[i], number_shots
            )
        )
        i = i + 1
    ...
    while i < len(counts_result):
        ...
        print(
            "------> Analysing result {0}. This result happened in {1:.4f} % of all cases\n".format(
                output_desired, prob_this_result
            )
        )
