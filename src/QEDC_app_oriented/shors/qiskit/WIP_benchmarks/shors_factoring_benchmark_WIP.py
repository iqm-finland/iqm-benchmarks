...
from utils import choose_random_base, determine_factors, generate_numbers, getAngle, getAngles, modinv
...
def ShorsAlgorithm(number, base, method, verbose=verbose):
    ...
    ma = [(measurements[2 * n - 1 - i] * 1.0 / (1 << (i + 1))) for i in range(2 * n)]
    y = sum(ma)
...
def analyze_and_print_result(qc, result, num_qubits, marked_item, num_shots):
    if verbose:
        print(f"For marked item {marked_item} measured: {result}")
...
def run(
    ...
    ex.set_execution_target(backend_id, provider_backend=provider_backend, hub=hub, group=group, project=project)
