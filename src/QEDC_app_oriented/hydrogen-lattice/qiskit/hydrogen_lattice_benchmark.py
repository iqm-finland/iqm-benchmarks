...
def HydrogenLattice(
    num_qubits, operator, secret_int=000000, thetas_array=None, parameterized=None, use_estimator=False
):
...
def run(
    ...
    if method == 1:
        metrics.plot_metrics(f"Benchmark Results - {benchmark_name} ({method}) - Qiskit", options=dict(shots=num_shots))
