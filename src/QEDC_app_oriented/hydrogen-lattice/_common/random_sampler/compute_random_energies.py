...
if min_qubits == max_qubits == 0:
    ...
    instance_filepath_list = [
        file for file in glob.glob(os.path.join(os.path.dirname(__file__), "..", common.INSTANCE_DIR, "h*_*_*_*.json"))
    ]
...
for qubit_count in np.arange(min_qubits, max_qubits + 2, 2):
    ...
    instance_filepath_list = [
        ...
        for file in glob.glob(
            os.path.join(os.path.dirname(__file__), "..", common.INSTANCE_DIR, f"h{qubit_count:03}_*_*_*.json")
        )
    ]
