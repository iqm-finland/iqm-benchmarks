...
def find_last_metrics_for_group(group, instance):
    ...
    for circuit_id in h_lattice_metrics[group]:
        if np.floor(int(circuit_id) / 1000) == instance:
            ...
            accuracy_ratio = (
                metrics_for_circuit['accuracy_ratio'] if 'accuracy_ratio' in metrics_for_circuit else solution_quality
            )
...
def plot_all_line_metrics(
    ...
    for qubit_count in group_keys:
        ...
        for instance in range(1, total_instances + 1):
            ...
            current_radius, doci_energy, fci_energy, random_energy, energy, solution_quality, accuracy_ratio = (
...
def plot_line_metric(
    ...
    _, _, standard_error_data, standard_error_label = find_metrics_array_for_group(
        group, instance, "standard_error", x_val, x_metric_name
    )
...
def plot_all_cumulative_metrics(
    ...
    for num_qubits in h_lattice_metrics:
        ...
        for instance in range(1, total_instances + 1):
            ...
            # search metrics store for final metrics for this group
            current_radius, doci_energy, fci_energy, random_energy, energy, accuracy_ratio, solution_quality = (
                find_last_metrics_for_group(group, instance)
            )

            ###### find the execution time array for "energy" metric
            x_data, x_label, y_data, y_label = find_metrics_array_for_group(
                group, instance, "energy", "cumulative_exec_time", "exec_time"
            )
            ...
            x_data, x_label, y_data, y_label = find_metrics_array_for_group(
                group, instance, "energy", "cumulative_elapsed_time", "elapsed_time"
            )
...
def plot_cumulative_metrics(
    ...
    # save the plot image
    if save_plot_images:
        metrics.save_plot_image(plt, os.path.join("Hydrogen-Lattice-(2)" + "-" + suffix), backend_id)
    # show the plot(s)
    plt.show(block=True)
