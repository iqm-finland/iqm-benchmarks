        ...
        for q in range(num_orbitals):
            if p != q:
                terms.append(
                    (
                        I(),
                        0.5 * two_body_integrals[p, p, q, q] + 0.25 * two_body_integrals[p, q, q, p],
                    )
                )
                terms.append(
                    (
    ...
    for shape in [chain]:
        for n in [2, 4, 6, 8, 10, 12, 14, 16]:
            for r in [0.75, 1.00, 1.25]:
                ...
                paired_qubit_hamiltonian = generate_paired_qubit_hamiltonian(fermionic_hamiltonian)

                jordan_wigner_hamiltonian = generate_jordan_wigner_hamiltonian(fermionic_hamiltonian)
                ...
                jordan_wigner_hamiltonian_dict = make_pauli_dict(jordan_wigner_hamiltonian)
