...
def get_instance_filepaths(num_qubits=2, radius=None):
    ...
    if radius is not None:
        ...
        instance_filepath = os.path.join(
            os.path.dirname(__file__), INSTANCE_DIR, f"h{num_qubits:03}_chain_{sradius}.json"
        )
    ...
    instance_filepath_list = [
        file
        for file in glob.glob(os.path.join(os.path.dirname(__file__), INSTANCE_DIR, f"h{num_qubits:03}_*_*_*.json"))
    ]
...
def get_random_energies_dict():
    ...
    random_energy_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "random_sampler/precomputed_random_energies.json"
    )
