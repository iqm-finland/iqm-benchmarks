...
def gaussian_dist(num_state_qubits, mu, sigma=0.3):
    ...
    for i in range(2**num_state_qubits):
        ...
        dist[key] = (
            0.5
            / normalization
            * (math.erf((b - mu) / (np.sqrt(2) * sigma)) - math.erf((a - mu) / (np.sqrt(2) * sigma)))
        )
    return dist
...
def mc_dist(num_counting_qubits, exact, c_star, method):
    ...
    for b in range(2**num_counting_qubits):
        ...
        if abs(phi - b / (2**num_counting_qubits)) > 1e-6:
            prob = (
                np.abs(
                    ((1 / 2) ** num_counting_qubits)
                    * (1 - np.exp(2j * np.pi * (2**num_counting_qubits * phi - b)))
                    / (1 - np.exp(2j * np.pi * (phi - b / (2**num_counting_qubits))))
                )
                ** 2
            )
        else:
