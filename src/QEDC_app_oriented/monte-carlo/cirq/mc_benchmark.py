...
def f_on_objective(qc, qr, f, epsilon=0.05, degree=3):
    ...
    for controls in b_exp.keys():
        ...
        if len(controls) == 0:
            ...
            qc.append(
                cirq.ry(-theta)
                .controlled(num_controls=len(controls))
    ...
    for r in regions:
        ...
        if r == '1':
            ...
            qc.append(
                ...
                .on(*controls + [qr[num_state_qubits - 1 - num_controls]])
            )
...
def AE_Subroutine(num_state_qubits, num_counting_qubits, A_circuit):
    ...
    if (Q_ or A_) == None or num_state_qubits <= 3:
        if num_state_qubits < 5:
            A_ = A
            Q_ = Q
    if QFTI_ == None or num_counting_qubits <= 3:
        if num_counting_qubits < 4:
...
def run(
    ...
    epsilon=0.05,
    degree=2,
    num_state_qubits=MIN_STATE_QUBITS,
    method=2,  # default, not exposed to users
    ...
    if method == 2:
        if max_qubits < MIN_QUBITS:
            print(
                f"INFO: Monte Carlo Simulation benchmark method ({method}) requires a minimum of {MIN_QUBITS} qubits."
            )
            return
        ...
        if max_qubits < MIN_QUBITS_M1:
            print(
                f"INFO: Monte Carlo Simulation benchmark method ({method}) requires a minimum of {MIN_QUBITS_M1} qubits."
            )
            return
    ...
    def execution_handler(qc, result, num_qubits, mu, num_shots):
        ...
        counts, fidelity = analyze_and_print_result(
            qc, result, num_counting_qubits, float(mu), num_shots, method=method, num_state_qubits=num_state_qubits
        )
    ...
    for num_qubits in range(min_qubits, max_qubits + 1):
        ...
        for mu in mu_range:
            ...
            qc = MonteCarloSampling(
                target_dist, f_to_estimate, num_state_qubits, num_counting_qubits, epsilon, degree, method=method
            )
    ...
    print("\nInverse QFT Circuit =")
    print(cirq.Circuit(cirq.decompose(QFTI_.on(*qr_state))) if QFTI_ != None else "  ... too large!")
