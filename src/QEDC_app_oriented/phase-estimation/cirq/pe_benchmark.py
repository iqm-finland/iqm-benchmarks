...
def analyze_and_print_result(qc, result, num_counting_qubits, theta, num_shots):
    ...
    if verbose:
        print(f"For theta value {theta}, measured: {counts}")
...
def run(min_qubits=3, max_qubits=8, max_circuits=3, num_shots=100, backend_id='simulator', provider_backend=None):
    ...
    for num_qubits in range(min_qubits, max_qubits + 1):
        ...
        if 2 ** (num_counting_qubits) <= max_circuits:
            ...
            theta_range = [
                ...
                for i in np.random.choice(2 ** (num_counting_qubits), num_circuits, False)
            ]
    ...
    qr_state = [
        cirq.GridQubit(i, 0) for i in range(QFTI_.num_qubits)
